...
@app.route('/post/<int:post_id>/editpost', methods=['GET', 'POST'])
def editpost(post_id):
    if 'username' not in login_session:
        return redirect('/login')
    editpost = session.query(Blogpost).filter_by(id=post_id).one()
    ...
    if request.method == 'POST':
        ...
        if request.form['subtitle']:
            editedItem.description = request.form['subtitle']
        if request.form['author']:
            editedItem.description = request.form['author']
        if request.form['content']:
            editedItem.description = request.form['content']
        session.add(editedItem)
        session.commit()
        return redirect(url_for('index', catalog_id=catalog_id))
    else:
        return render_template('index.html', item=editedItem)
